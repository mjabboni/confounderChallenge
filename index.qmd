...
subtitle: "A Data-Driven Analysis of Confounding and Causal Inference in Surgical Outcomes"
format:
  html: default
...
ax.scatter(dreamy_df['patient'], dreamy_df['post_surgical_score'], 
           marker='o', s=100, color='#4E79A7', label='Doc Dreamy', alpha=0.7, edgecolors='black', linewidth=0.5)
ax.scatter(duck_df['patient'], duck_df['post_surgical_score'], 
           marker='^', s=100, color='#E15759', label='Doc Duck', alpha=0.7, edgecolors='black', linewidth=0.5)

# Add horizontal mean lines
ax.axhline(mean_dreamy, color='#4E79A7', linestyle='--', linewidth=2, alpha=0.8)
ax.axhline(mean_duck, color='#E15759', linestyle='--', linewidth=2, alpha=0.8)

# Add text annotations with symbols in square boxes
ax.text(5, mean_dreamy, f'○ Doc Dreamy Mean: {mean_dreamy:.2f}', 
        fontsize=11, color='#4E79A7', fontweight='bold', va='center',
        bbox=dict(boxstyle='square', facecolor='white', edgecolor='#4E79A7', alpha=0.9, linewidth=2))
ax.text(5, mean_duck, f'△ Doc Duck Mean: {mean_duck:.2f}', 
        fontsize=11, color='#E15759', fontweight='bold', va='center',
        bbox=dict(boxstyle='square', facecolor='white', edgecolor='#E15759', alpha=0.9, linewidth=2))

# Styling
ax.set_xlabel('Patient Number', fontsize=12, fontweight='bold')
ax.set_ylabel('Post-Surgical Score\n(Lower is Better)', fontsize=12, fontweight='bold')
...
mean_dreamy_rand = dreamy_rand_df['post_surgical_score'].mean()
mean_duck_rand = duck_rand_df['post_surgical_score'].mean()
...
# Add horizontal mean lines
ax.axhline(mean_dreamy_rand, color='#4E79A7', linestyle='--', linewidth=2, alpha=0.8)
ax.axhline(mean_duck_rand, color='#E15759', linestyle='--', linewidth=2, alpha=0.8)

# Add text annotations with symbols in square boxes
ax.text(5, mean_dreamy_rand, f'○ Doc Dreamy Mean: {mean_dreamy_rand:.2f}', 
        fontsize=11, color='#4E79A7', fontweight='bold', va='center',
        bbox=dict(boxstyle='square', facecolor='white', edgecolor='#4E79A7', alpha=0.9, linewidth=2))
ax.text(5, mean_duck_rand, f'△ Doc Duck Mean: {mean_duck_rand:.2f}', 
        fontsize=11, color='#E15759', fontweight='bold', va='center',
        bbox=dict(boxstyle='square', facecolor='white', edgecolor='#E15759', alpha=0.9, linewidth=2))
...
# Filter data to only include patients with severity between -1 and 1 (the overlapping region)
patients_subset = patients_df[(patients_df['severity'] >= -1) & (patients_df['severity'] <= 1)]

# Calculate means for each doctor using the subset
dreamy_subset = patients_subset[patients_subset['doctor_name'] == 'Doc Dreamy']
duck_subset = patients_subset[patients_subset['doctor_name'] == 'Doc Duck']

mean_dreamy_subset = dreamy_subset['post_surgical_score'].mean()
mean_duck_subset = duck_subset['post_surgical_score'].mean()

# Create figure
fig, ax = plt.subplots(figsize=(8, 8))

# Add gradient shading to represent the common cause confounder (severity)
# Create multiple vertical bands with colors representing severity levels
# This visually shows how severity (the confounder) stratifies the data
severity_bands = np.linspace(patients_subset['severity'].min(), patients_subset['severity'].max(), 50)
cmap_background = plt.cm.RdYlGn_r
for i in range(len(severity_bands)-1):
    band_start = severity_bands[i]
    band_end = severity_bands[i+1]
    # Normalize the color based on severity
    color_intensity = (band_start - severity_bands.min()) / (severity_bands.max() - severity_bands.min())
    ax.axvspan(band_start, band_end, alpha=0.12, color=cmap_background(color_intensity), zorder=0)

# Add darker highlight for the overlapping region (-1 to 1)
ax.axvspan(-1, 1, alpha=0.08, color='black', zorder=1, linestyle='--', linewidth=2, edgecolor='black')

# Create colormap for severity gradient (darker = more severe)
# Using RdYlGn_r (reversed Red-Yellow-Green) so red = high severity, green = low severity
cmap = plt.cm.RdYlGn_r

# Normalize severity values for colormap based on the subset range
severity_min = patients_subset['severity'].min()
severity_max = patients_subset['severity'].max()
norm = plt.Normalize(vmin=severity_min, vmax=severity_max)

# Plot scatter points with color gradient based on severity (only subset)
scatter_dreamy = ax.scatter(dreamy_subset['severity'], dreamy_subset['post_surgical_score'], 
           marker='o', s=100, c=dreamy_subset['severity'], cmap=cmap, norm=norm,
           label='Doc Dreamy', alpha=0.8, edgecolors='black', linewidth=0.5, zorder=3)
scatter_duck = ax.scatter(duck_subset['severity'], duck_subset['post_surgical_score'], 
           marker='^', s=100, c=duck_subset['severity'], cmap=cmap, norm=norm,
           label='Doc Duck', alpha=0.8, edgecolors='black', linewidth=0.5, zorder=3)

# Add colorbar to show severity gradient
cbar = plt.colorbar(scatter_dreamy, ax=ax, pad=0.02)
cbar.set_label('Patient Severity\n(Higher = More Severe)', rotation=270, labelpad=20, fontsize=11, fontweight='bold')

# Add horizontal mean lines for the subset
ax.axhline(mean_dreamy_subset, color='#4E79A7', linestyle='--', linewidth=2, alpha=0.8, zorder=2)
ax.axhline(mean_duck_subset, color='#E15759', linestyle='--', linewidth=2, alpha=0.8, zorder=2)

# Add text annotations with symbols in square boxes (positioned on the left side)
ax.text(-0.9, mean_dreamy_subset, f'○ Doc Dreamy Mean: {mean_dreamy_subset:.2f}', 
        fontsize=11, color='#4E79A7', fontweight='bold', va='center',
        bbox=dict(boxstyle='square', facecolor='white', edgecolor='#4E79A7', alpha=0.9, linewidth=2))
ax.text(-0.9, mean_duck_subset, f'△ Doc Duck Mean: {mean_duck_subset:.2f}', 
        fontsize=11, color='#E15759', fontweight='bold', va='center',
        bbox=dict(boxstyle='square', facecolor='white', edgecolor='#E15759', alpha=0.9, linewidth=2))
...
ax.set_ylabel('Post-Surgical Score\n(Lower is Better)', fontsize=12, fontweight='bold')
ax.set_title('Post-Surgical Symptom Score by Patient Severity\n(Stratified Subset: Severity -1 to 1)', fontsize=14, fontweight='bold', pad=20)
ax.legend(loc='upper right', fontsize=11, frameon=True, shadow=True)
ax.grid(True, alpha=0.3, linestyle='-', linewidth=0.5)
...
This analysis demonstrates the critical importance of understanding confounding in observational data. When we initially examined outcomes between the two surgeons using observational data, Doc Dreamy appeared superior with a lower average post-surgical symptom score (2.8 vs. 3.38). However, this conclusion was misleading due to a hidden confounder: patient severity.

The confounding arose because patients chose their surgeon based on availability, with sicker patients (who needed urgent care) selecting the more available Doc Duck, while healthier patients chose the seemingly more prestigious Doc Dreamy. This created a spurious association between surgeon choice and outcomes.

We explored two solutions to this confounding problem:

1. **Randomization**: By randomly assigning patients to surgeons, we broke the confounding pathway. The randomized trial revealed the truth: Doc Duck is actually the better surgeon, with patients averaging a score of 2.71 compared to Doc Dreamy's 3.46.

2. **Stratification**: When randomization isn't possible, we can control for confounders by examining outcomes within groups of similar severity. Our stratified analysis visually confirmed that, among patients of comparable severity, Doc Duck consistently achieved better outcomes.

This case study reinforces a fundamental principle: appearances can be deceiving, and raw statistical associations don't necessarily reflect causal relationships. Only through careful consideration of confounding and appropriate analytical techniques—whether randomization or stratification—can we uncover the true causal effects hidden in observational data.
